...
reference if the label contains it, otherwise the property specified by the
\opt{ref} option is used as fallback.  This is useful for things like
sub-elements for which we can build a proper abbreviated sub-reference and
populate the label with it (the \texttt{memoir} and \texttt{subcaption}
compatibility modules already provide the properties \texttt{subcaption} and
\texttt{subref}, respectively, but similar ones can be built with \pkg{zref},
as needed).
