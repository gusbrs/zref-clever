...
\begin{zchowto}
\caption{\cs{newtheorem}, simple case}
\begin{lstlisting}
...
\begin{zchowto}
\caption{\cs{newtheorem}, shared counter}
\begin{lstlisting}
\documentclass{article}
\usepackage{zref-clever}
...
\begin{zchowto}
\caption{\cs{newtheorem}, custom type}
\begin{lstlisting}
\documentclass{article}
\usepackage{zref-clever}
...
`\textbackslash{}'begin{lstlisting}[caption={Useless code}, label=lst:1]
...
for a use example.  This module also ensures proper \opt{currentcounter}
values are in place for the display math environments, for which it uses
environment hooks, and sets the font of equation references to \cs{upshape},
following \pkg{amsmath}'s \cs{eqref}.
...
the \pkg{listings}' counters: \texttt{lstlisting} and \texttt{lstnumber}.
...
`egreg', Steven B.\ Segletes, \texttt{@samcarter}, Alan Munn, Florent Rougon,
David Carlisle.
...
at \url{https://github.com/gusbrs/zref-clever/blob/main/CHANGELOG.md}.
