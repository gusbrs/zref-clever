...
\zctask{Typesetting the references as ordinal numbers with \cs{zcref}.}

This is intended as an example of the flexibility \pkg{zref}'s extensible
referencing system grants us.\footnote{Though clearly simplified, the example
  is less than academic.  See \url{https://tex.stackexchange.com/a/670088} for
  an application to add a reference suffix needed in Turkish.}  This can be
done with a simple custom \pkg{zref} property, which can then be used to set
\pkg{zref-clever}'s \opt{ref} option.
