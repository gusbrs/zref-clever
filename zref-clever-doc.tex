...
\pkg{zref-clever} requires a recent \LaTeX{} kernel
(\makeatletter\zc@required@kernel@version\makeatother{} or newer) and the
packages \pkg{zref}, including some of its modules, and \pkg{ifdraft}.  It
...
\pkg{zref-clever} requires \pkg{zref}, and \LaTeX{} kernel
\makeatletter\zc@required@kernel@version\makeatother{}, or newer.  It also
needs \pkg{ifdraft}.  Some packages are leveraged by \pkg{zref-clever} if they
are present, but are not loaded by default or required by it, namely:
\pkg{hyperref}, \pkg{zref-check}, and \pkg{zref}'s \pkg{zref-titleref},
\pkg{zref-hyperref}, and \pkg{zref-xr} modules.
